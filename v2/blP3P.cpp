...
void blP3P(bl::vP3D points3D, bl::vP2F points2D, const cv::Mat cameraMatrix, const cv::Mat distCoeffs, cv::Mat& R, cv::Mat& T)
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		// 归一化到数学坐标
		double norm = sqrt(temp.x * temp.x + temp.y * temp.y + 1); // 求模长
		temp.z = 1.0 / norm;    // 转换为单位向量
		temp.x *= temp.z;
		temp.y *= temp.z;
  ...
  //  std::cout << "distance: " << distances[0] << " " << distances[1] << " " << distances[2] << std::endl;
	...
	for (int i = 0; i < n; i++)
	{
		...
		double M_orig[3][3];
		for (int i = 0; i < 3; i++) {
			M_orig[i][0] = points3D[i].x;
			M_orig[i][1] = points3D[i].y;
			M_orig[i][2] = points3D[i].z;
		}

		if (!align(M_orig, camPoints[0].x, camPoints[0].y, camPoints[0].z, camPoints[1].x, camPoints[1].y, camPoints[1].z,
			camPoints[2].x, camPoints[2].y, camPoints[2].z, r[nb_solutions], t[nb_solutions]))
			continue;


		cv::Mat t_r = cv::Mat(3, 3, CV_64F, r[nb_solutions]);
		cv::Mat t_t = cv::Mat(3, 1, CV_64F, t[nb_solutions]);

		bl::R_T2H(t_r, t_t, h[i]);
